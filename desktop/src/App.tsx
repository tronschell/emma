  ...
  return (
    <div className="app-shell" style={shellStyle}>
      ...
      <aside className={`nav-rail ${layout.navCollapsed ? "collapsed" : ""}`} aria-label="Workspace navigation">
        ...
        <div className="nav-foot"><span><i /> AGENT ONLINE</span><small>⌘ ⇧ SPACE FOR QUICK ASK</small></div>
...
const providerMarks = [
  ["openai", "◎", "OpenAI", "US"], ["anthropic", "A", "Anthropic", "US"], ["gemini", "✦", "Gemini", "US"], ["xai", "X", "xAI", "US"],
  ["meta", "∞", "Meta", "US"], ["mistral", "M", "Mistral", "EU"], ["cohere", "C", "Cohere", "CA"], ["qwen", "Q", "Qwen", "CN"],
  ["deepseek", "D", "DeepSeek", "CN"], ["kimi", "K", "Kimi", "CN"], ["glm", "Z", "Z.ai / GLM", "CN"], ["minimax", "M", "MiniMax", "CN"],
  ["ernie", "E", "ERNIE", "CN"], ["hunyuan", "H", "Hunyuan", "CN"], ["naver", "N", "HyperCLOVA", "KR"], ["sakana", "S", "Sakana AI", "JP"],
] as const;

function ProviderMarks() {
  return <section className="provider-marks"><header><div><span>PROVIDER PLUGINS</span><h3>OpenAI-compatible routing</h3></div><small>NEUTRAL MARKS · OFFICIAL ASSETS REQUIRE BRAND RIGHTS</small></header><div>{providerMarks.map(([id, mark, name, region]) => <span key={id}><b className={`provider-mark ${id}`}>{mark}</b><strong>{name}</strong><small>{region}</small></span>)}</div></section>;
}
...
function Overlay() {
  ...
  const runAction = useCallback(async (index: number) => {
    ...
    try {
      ...
      const destination = action.destinationKnowledgeBaseId || snapshot.knowledgeBases[0]?.id || "default";
