...
export default function AgentView({ snapshot, act, busy, openThread, projectName, mode, model }: { snapshot: Snapshot; act: Act; busy: boolean; openThread: (id: string) => void; projectName: (thread: Thread) => string; mode: string; model: string }) {
  ...
  const decide = (item: Improvement, state: Improvement["state"], result: string) =>
    save(store.items.map((row) => row.id === item.id ? { ...row, state, decidedAt: Date.now(), result } : row));
  ...
  return <section className="agent-view">
    ...
    <section className="evidence-table">
      <header>
        ...
        <small>{WINDOW_DAYS} days · {read} recent {plural(read, "thread")} read</small>
      </header>
      {friction.map((item) => {
        const looks = priors(draftProposal(item).title);
        return <details key={item.key}>
          ...
          <div className="agent-actions">
            <button type="button" disabled={busy || !!trial || draft?.key === item.key} onClick={() => propose(item)}>{trial ? "A trial is already running" : looks.length ? "Propose a new change" : "Propose a change"}</button>
      ...
      {!friction.length && <Empty copy={ready
        ? turns.length ? "Nothing repeated twice." : "No finished turns yet."
        : "Reading traces…"} />}
    </section>
