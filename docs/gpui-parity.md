...
| Activity/machine/worktrees/update/platform | `activity.test.ts`, `machine.test.ts`, `worktrees` behavior is covered through Git/shared tests and needs new UI tests, `update.test.ts`, `windows.test.mjs`, `release.test.mjs`, `conversation-css.test.mjs`. |
