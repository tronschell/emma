...
fn buildFooterSurfaceProjection(
    ...
    const show_model_query = !viewer_active and !modal_active and !ctx.stream.active and ctx.model_query_active;
    ...
    const show_slash_query = geometry.show_slash_query;
    const show_picker = show_model_query or show_file_query or show_slash_query;
...
test "surface footer measurement reserves capped picker rows for active list pickers" {
    ...
    try slash_input.edit_state.input.appendSlice(alloc, "/he");
