...
pub const TextEmission = union(enum) {
    ...
    /// The model's reasoning stream. Surfaces that fold thinking away separately
    /// from the answer take this; the rest drop it.
    reasoning: []const u8,
};
...
pub const AgentRuntimeDeps = struct {
    ...
    /// A context experiment fired on this step. Not a notice: it says nothing to
