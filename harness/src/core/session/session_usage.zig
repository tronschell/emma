...
/// Cumulative token counts, for callers that want the numbers and not a whole
/// `Snapshot`. Subtract two of these to get what one turn cost.
pub const TokenTotals = struct {
    input_tokens: u64 = 0,
    output_tokens: u64 = 0,

    pub fn since(self: TokenTotals, earlier: TokenTotals) TokenTotals {
        return .{
...
pub const Usage = struct {
    ...
    /// Session token totals so far, without the allocation `snapshot` costs.
    /// Locked, because a reconciliation thread may still be folding in a late
    /// provider report while a caller reads.
    pub fn tokenTotals(self: *Usage) TokenTotals {
        self.mutex.lockUncancelable(io_mod.getIo());
        defer self.mutex.unlock(io_mod.getIo());
...
test "pending generation origins remain bounded transport inputs" {
    try validateOrigin("https://openrouter.ai/api");
